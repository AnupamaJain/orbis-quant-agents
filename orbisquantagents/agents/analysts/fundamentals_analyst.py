...
def create_fundamentals_analyst(llm):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            + " Use the available tools: `get_fundamentals` for comprehensive company analysis, `get_balance_sheet`, `get_cashflow`, and `get_income_statement` for specific financial statements. For Indian stocks, use `get_sebi_filings` to check recent corporate announcements and `get_bulk_block_deals` to identify significant institutional interest (FII/DII activity)."
            + get_language_instruction(),
        )
