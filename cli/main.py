...
app = typer.Typer(
    name="orbisquantagents",
    help="Orbis Quant Agents CLI: Multi-Agents LLM Financial Trading Framework",
...
class MessageBuffer:
    ...
    REPORT_SECTIONS = {
        ...
        "small_cap_report": ("small_cap", "Small Cap & PSU Analyst"),
    ...
    def _update_current_report(self):
        ...
        if latest_section and latest_content:
            ...
            section_titles = {
                ...
                "small_cap_report": "Small Cap & PSU Analysis",
    ...
    def _update_final_report(self):
        ...
        if any(self.report_sections.get(section) for section in analyst_sections):
            ...
            if self.report_sections.get("small_cap_report"):
                report_parts.append(
                    f"### Small Cap & PSU Analysis\n{self.report_sections['small_cap_report']}"
                )
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    layout["header"].update(
        Panel(
            "[bold green]Welcome to Orbis Quant Agents CLI[/bold green]\n"
            "[dim]© [Orbis Quant AI](https://github.com/OrbisQuantAI)[/dim]",
            title="Welcome to Orbis Quant Agents",
            border_style="green",
            padding=(1, 2),
            expand=True,
        )
...
def get_user_selections():
    ...
    welcome_content += "[bold green]Orbis Quant Agents: Multi-Agents LLM Financial Trading Framework - CLI[/bold green]\n\n"
    welcome_content += "[bold]Workflow Steps:[/bold]\n"
    welcome_content += "I. Analyst Team → II. Research Team → III. Trader → IV. Risk Management → V. Portfolio Management\n\n"
    welcome_content += (
        "[dim]Built by [Orbis Quant AI](https://github.com/OrbisQuantAI)[/dim]"
    )
    ...
    console.print(
        create_question_box(
            "Step 1: Ticker Symbol",
            "Enter the exact ticker symbol to analyze, including exchange suffix when needed (examples: RELIANCE.NS, HDFCBANK.NS, TCS.NS, INFEY.NS)",
            "RELIANCE.NS",
        )
    )
...
def get_ticker():
    ...
    return typer.prompt("", default="RELIANCE.NS")
...
def save_report_to_disk(final_state, ticker: str, save_path: Path):
    ...
    if final_state.get("small_cap_report"):
        analysts_dir.mkdir(exist_ok=True)
        (analysts_dir / "small_cap.md").write_text(final_state["small_cap_report"])
        analyst_parts.append(("Small Cap & PSU Analyst", final_state["small_cap_report"]))
    if analyst_parts:
...
def display_complete_report(final_state):
    ...
    if final_state.get("small_cap_report"):
        analysts.append(("Small Cap & PSU Analyst", final_state["small_cap_report"]))
    if analysts:
...
ANALYST_ORDER = ["market", "social", "news", "fundamentals", "small_cap"]
ANALYST_AGENT_NAMES = {
    "market": "Market Analyst",
    "social": "Social Analyst",
    "news": "News Analyst",
